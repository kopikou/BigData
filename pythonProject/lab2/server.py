...
def get_clients():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM client_registration_card ORDER BY id DESC;")
...
def get_seats():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM seat  ORDER BY id DESC;")
...
def get_accommodations():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM accommodation  ORDER BY accommodation_id DESC;")
...
@app.route('/current_residence', methods=['GET'])
def get_current_residence():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM accommodation WHERE end_date > GETDATE();")

    current_residences = cursor.fetchall()

    cursor.close()
    conn.close()
    return jsonify([{"accommodation_id": current_residence.accommodation_id, "client_id": current_residence.client_id, "room_id": current_residence.room_id,
                     "start_date": current_residence.start_date, "end_date": current_residence.end_date} for current_residence in current_residences])
@app.route('/accommodation_client', methods=['GET'])
def get_accommodation_client():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT acc.*, cl.full_name, cl.phone FROM accommodation acc INNER JOIN client_registration_card cl ON acc.client_id = cl.id WHERE end_date > GETDATE();;")
    ...
    conn.close()
    return jsonify([{"accommodation_id": accommodation.accommodation_id, "client_id": accommodation.client_id, "room_id": accommodation.room_id,
                     "start_date": accommodation.start_date, "end_date": accommodation.end_date, "full_name": accommodation.full_name,
                     "phone": accommodation.phone} for accommodation in accommodations])
@app.route('/accommodation_comfort/<int:comfort>', methods=['GET'])
def accommodation_comfort(comfort):
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT acc.*, r.type_of_gender, r.number_of_seats, r.type_comfort, r.hostel_id FROM accommodation acc INNER JOIN rooms r ON acc.room_id = r.id WHERE r.type_comfort = ?;", comfort)
    ...
    conn.close()
    return jsonify([{"accommodation_id": accommodation.accommodation_id, "client_id": accommodation.client_id, "room_id": accommodation.room_id,
                     "start_date": accommodation.start_date, "end_date": accommodation.end_date, "type_of_gender": accommodation.type_of_gender,
                     "number_of_seats": accommodation.number_of_seats,"type_comfort": accommodation.type_comfort,
                     "hostel_id": accommodation.hostel_id} for accommodation in accommodations])
@app.route('/cnt_residence_by_day', methods=['GET'])
def get_cnt_residence_by_day():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT CAST(start_date AS DATE) AS day, COUNT(*) AS number_of_guests FROM accommodation GROUP BY CAST(start_date AS DATE) ORDER BY CAST(start_date AS DATE);")

    accommodations = cursor.fetchall()

    cursor.close()
    conn.close()
    return jsonify([{"day": accommodation.day, "number_of_guests": accommodation.number_of_guests} for accommodation in accommodations])
@app.route('/client_gender/<string:gender>', methods=['GET'])
def get_client_gender(gender):
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM client_registration_card WHERE CAST(gender AS varchar(10)) = '?';", gender)

    clients = cursor.fetchall()

    cursor.close()
    conn.close()
    return jsonify([{"id": client.id, "full_name": client.full_name, "phone": client.phone,
                     "gender": client.gender, "employee_id": client.employee_id} for client in clients])
@app.route('/cnt_clients_of_employee', methods=['GET'])
def get_cnt_clients_of_employee():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT CAST(e.full_name AS varchar(100)) AS full_name, COUNT(*) AS count_clients FROM client_registration_card cr INNER JOIN employee e ON cr.employee_id = e.id GROUP BY CAST(e.full_name AS varchar(100)) ORDER BY COUNT(*) DESC;")

    datas = cursor.fetchall()

    cursor.close()
    conn.close()
    return jsonify([{"full_name": data.full_name, "count_clients": data.count_clients} for data in datas])
@app.route('/reservations_last_month', methods=['GET'])
def get_reservations_last_month():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM reservation WHERE start_date >= DATEADD(MONTH, -1, CAST(GETDATE() AS DATE));")
    ...
    return jsonify([{"client_id": reservation.client_id, "seat_id": reservation.seat_id,
                     "start_date": reservation.start_date, "end_date": reservation.end_date} for reservation in reservations])
@app.route('/cnt_reservations_client', methods=['GET'])
def get_cnt_reservations_client():
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute("SELECT CAST(cl.full_name AS varchar(100)) AS full_name, COUNT(*) AS reservations_count FROM reservation r INNER JOIN client_registration_card cl ON r.client_id = cl.id GROUP BY CAST(cl.full_name AS varchar(100)) ORDER BY reservations_count DESC;")

    reservations = cursor.fetchall()

    cursor.close()
    conn.close()
    return jsonify([{"full_name": reservation.full_name, "reservations_count": reservation.reservations_count} for reservation in reservations])
